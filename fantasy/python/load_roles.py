...
class LoadData(PythonTask):
    ...
    def run(self):
        ...
        with self.step('Get team URLs'):
            ...
            teams = soup.find_all(
                'div', class_='teamcard toggle-area toggle-area-1')
            team_urls = [team.find_next('a').get_attribute_list('href')[
                                        0] for team in teams]
        ...
        with self.step('Get player roles'):
            ...
            for team_url in team_urls:
                ...
                soup = BeautifulSoup(page.text, 'html.parser')
                roster_table = soup.find(
                    'table', class_='wikitable wikitable-striped roster-card')
                player_names = roster_table.find_all('span', {"id": "player"})
                roles = roster_table.find_all(
                    'td', {"class": "PositionWoTeam2"})
                if not roles:
                    roles = roster_table.find_all('td', {"class": "Position"})
                player_tags = [player.find('a').get_attribute_list('title')[
                                           0] for player in player_names]
